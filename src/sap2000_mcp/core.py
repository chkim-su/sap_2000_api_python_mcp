...
from .scoring import ScoringInput, ScoringWeights, score_candidate
from .text_utils import normalize_for_index, generate_char_ngrams
from .vocabulary import group_by_domain, expand_query
...
def connect() -> sqlite3.Connection:
    """Connect to packaged or WORK_DIR database."""
    if DB.conn is None:
        work_path = DEFAULT_ENV.work_dir / "db" / "sap2000_mcp.db"
        candidate: Optional[str] = None
        if work_path.exists():
            candidate = str(work_path)
        else:
            # packaged DB
            ...
            try:
                ...
                candidate = None
        if not candidate:
            raise RuntimeError("No database found. Provide packaged DB or build one in WORK_DIR.")
        DB.conn = sqlite3.connect(candidate)
...
def _get_function(fid: int) -> Optional[sqlite3.Row]:
    conn = connect()
    cur = conn.execute("SELECT * FROM functions WHERE id = ?", (fid,))
    return cur.fetchone()
